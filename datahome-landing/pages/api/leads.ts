...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  const { name, email, phone, message, plan, source, locale, metadata } = (req.body || {}) as Record<
    string,
    unknown
  >
  ...
  const html = buildHtml({
    ...
    page_url: metadata && typeof metadata === "object" ? (metadata as Record<string, string>).page_url : undefined,
    timestamp,
  })
